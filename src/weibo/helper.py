...
def is_square_like_image(img_path: str, tolerance_percent: float = 5.0) -> bool:
    ...
    try:
        ...
        debug_log(f"图片比例检查: {width}x{height}, 比例={aspect_ratio:.2f}, 阈值={tolerance_ratio:.2f}")
        
        # 如果长宽比在容忍范围内，认为是接近1:1的图片
        return aspect_ratio <= tolerance_ratio
        
    except Exception as e:
        debug_log(f"检查1:1图片失败: {e}")
        return False
...
def merge_square_like_images(image_paths: List[str], cache_dir: str, tolerance_percent: float = 5.0) -> Tuple[Optional[str], List[str]]:
    ...
    try:
        ...
        if len(square_like_paths) < 2:
            debug_log(f"接近1:1图片数量不足({len(square_like_paths)}张)，跳过拼接")
            return None, image_paths
        
        debug_log(f"找到{len(square_like_paths)}张接近1:1图片，开始拼接")
        ...
        if len(square_like_paths) >= 4:
            ...
            if merged_path:
                debug_log(f"2x2网格拼接完成，使用了{used_count}张图片")
        ...
        debug_log(f"1:1图片拼接失败: {e}", force=True)
        return None, image_paths
...
def merge_square_grid(image_paths: List[str], cache_dir: str, grid_size: Tuple[int, int]) -> Tuple[Optional[str], int]:
    ...
    try:
        ...
        debug_log(f"1:1图片{rows}x{cols}网格拼接完成: {merged_path}")
        debug_log(f"拼接尺寸: {canvas_width}x{canvas_height}")
        
        return merged_path, len(processed_images)
        
    except Exception as e:
        debug_log(f"1:1图片网格拼接失败: {e}", force=True)
        # 清理已加载的图片对象
        try:
...
def merge_remaining_long_strips(image_paths: List[str], cache_dir: str, 
    ...
    try:
        ...
        if len(long_strip_paths) < 2:
            debug_log(f"长条图数量不足({len(long_strip_paths)}张)，跳过拼接")
            return None, image_paths
        
        debug_log(f"找到{len(long_strip_paths)}张长条图，开始左右拼接")
        ...
        # 计算所有图片按比例缩放后的宽度总和
        scaled_images = []
        actual_max_height = 0  # 记录实际缩放后的最大高度
        for img, width, height in long_strip_images:
            ...
            if new_width > 0 and new_height > 0:
                ...
                actual_max_height = max(actual_max_height, new_height)  # 使用实际图片高度
        ...
        if total_width > max_width:
            ...
            debug_log(f"拼接宽度过大({total_width}px)，重新缩放({width_scale:.2f})")
            ...
            scaled_images = []
            total_width = 0
            actual_max_height = 0  # 记录实际图片的最大高度
            ...
            max_height = actual_max_height  # 使用实际最大高度而不是计算值
        ...
        debug_log(f"长条图拼接完成: {len(long_strip_paths)}张 -> {merged_path}")
        debug_log(f"拼接尺寸: {total_width}x{max_height}")
        
        # 返回拼接图片路径和剩余的普通图片
        return merged_path, normal_image_paths
        
    except Exception as e:
        debug_log(f"长条图拼接失败: {e}", force=True)
        # 清理已打开的图片对象
        try:
            for img in scaled_images:
                img.close()
