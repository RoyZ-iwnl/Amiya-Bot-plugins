...
def load_headers_from_json() -> dict:
    with open(HEADERS_PATH, "r", encoding="utf-8") as f:
        headers = json.load(f)
    # 过滤掉:开头的key，仅保留常规headers（如有需要可自行修改）
...
class WeiboUser:
    def __init__(self, weibo_id, setting):
        ...
        self.headers = load_headers_from_json()
        # 强制设置Referer为当前用户页面，防止风控
        self.headers['Referer'] = f'https://m.weibo.cn/u/{weibo_id}'
